...
import com.hred.service.descriptors.outputDescriptor.EmployeeListOutputDiscriptor;
...
@Path("/viewEmployee")
@UnSecure
public String viewEmployee(@Context HttpHeaders headers, @Context UriInfo uriInfo,
WebserviceRequest request) throws ObjectNotFoundException,
BusinessException, EncryptionException, EmployeeException {

Employee employee = (Employee) JsonUtil.getObject(request.getPayload(),
Employee.class);
List<Employee> employees = (List<Employee>) EmployeeHandler.getInstance().viewEmployee(employee);
System.out.println("COunt : "+ employees.size());
//String outputString = "{\"status\": \"SUCCESS\", \"payload\": \"Test Successful\"}";
return JsonUtil.getJsonForListBasedOnDescriptor(employees, Employee.class, EmployeeListOutputDiscriptor.class);
}
